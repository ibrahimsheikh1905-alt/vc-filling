...
const StepOne = () => {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<Inputs>({ defaultValues: { agentChangeOption: "false" } });
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    updateFormData({...data, agentChangeOption: data.agentChangeOption === "true"});
    router.push("/forms/step-final");
  };
  console.log(watch("agentChangeOption"));
  ...
  useEffect(() => {
    ...
    const setServiceType = () => {
      localStorage.setItem("serviceType", "registered-agent");
    };
    setServiceType();
  }, []);
        ...
        <div className="container my-10 ">
          <div className="md:flex gap-10 ">
            <div className="max-w-4xl mx-auto">
              <h1 className="text-3xl font-bold">Registered Agent Service</h1>
              <p className="text-gray-600">
                Every state requires a corporation or an LLC to have a
                Registered Agent (sometimes called a resident agent, statutory
                agent, or agent for service of process). The Registered Agent
                address is the address that will be used by the state for any
              ...
              <div className=" bg-white rounded-3xl mx-auto max-w-4xl shadow-lg px-16 py-12 my-8">
                <h2 className="text-2xl font-bold mb-6">Change of Agent</h2>
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                    Would you like us to facilitate the transfer of Registered
                    Agent service from your current provider to us? *
                  </label>
                  <div className="my-3 flex gap-8 items-center">
                      ...
                      <div className="flex gap-2 items-center">
                        <div
                          className={`min-w-4 max-w-4 h-4 rounded-full border-2 ${
                            formData.agentChangeOption === "true"
                        ...
                        <input
                          ...
                          className="hidden"
                          value={"true"}
                          {...register("agentChangeOption", {
                            required: true,
                            onChange: (e) => {
                              updateFormData({
                                agentChangeOption: e.target.value,
                              });
                            },
                          })}
                        />
                    ...
                    <label className="cursor-pointer">
                      <div className="flex gap-2 items-center">
                        <div
                          className={`min-w-4 max-w-4 h-4 rounded-full border-2  ${
                            formData.agentChangeOption === "false"
                              ? "border-primary bg-primary"
                              : "border-gray-300"
                          }`}
                        >
                  ...
                  <p className="text-gray-500">
                    Select No, if entity has not been filled.
                  </p>
                  <div className="my-6 bg-gray-100 p-6 rounded-lg flex items-center">
                    <LightBulbIcon className="min-h-8 min-w-8 max-h-8 max-w-8  mr-8 text-primary" />
                    <p>
                      If the entity has a current registered agent on record it
                      is mandatory that the designated forms to transfer the
                      service to the new registered agent are filed. Otherwise,
                      the state will not recognize that a new registered agent
                      has been assigned. As a courtesy, we offer our clients the
                      option to have us file the required documents on their
                      behalf.
                    </p>
                  </div>
                </div>
