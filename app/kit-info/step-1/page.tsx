...
const StepOne = () => {
  ...
  const handleAutoLogin = async (firstName: string, email: string) => {
    if (!firstName || !email || autoLoginAttempted) return;
    ...
    try {
      ...
      // Also store in localStorage for the form data
      updateFormData({ 
        firstName: data.user.name?.split(" ")[0] || firstName,
        lastName: data.user.name?.split(" ")[1] || formData.lastName,
        email: email
      });

      setAuthStatus(data.isNewUser ? "Account created!" : "Logged in!");
  ...
  // Check if we should attempt auto-login when firstName or email changes
  const checkAndAutoLogin = (newFirstName: string, newEmail: string) => {
    if (newFirstName && newEmail && !autoLoginAttempted && !isLoading) {
      handleAutoLogin(newFirstName, newEmail);
    }
  };
                        ...
                        <input
                          ...
                          {...register("firstName", {
                            ...
                            onChange: (e) => {
                              ...
                              updateFormData({ firstName: newValue });
                              // Trigger auto-login check
                              checkAndAutoLogin(newValue, formData.email);
                            },
                          })}
                          ...
                          <input
                            ...
                            {...register("email", {
                              ...
                              onChange: (e) => {
                                ...
                                // Trigger auto-login check
                                checkAndAutoLogin(formData.firstName, newValue);
                              },
                            })}
