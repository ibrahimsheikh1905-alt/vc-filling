  ...
  try {
    ...
    try {
      ...
      if (userResponse.user) {
        await handleUsers(userResponse.user, "form_s_corporation");
      }
    } catch (error) {
    ...
    const query = `
      INSERT INTO form_s_corporation (
        ${fields.join(", ")}
      ) VALUES (
        ${fields.map(() => "?").join(", ")}
      );
    ...
    await executeQuery(newQuery, [
      ...
      data.clientEmail ?? null,
      data.clientPhoneNumber ?? null,
      `form_s_corporation-${insertedId}`,
      getFormattedDate(),
    ]);

    return NextResponse.json({
      message: "Data inserted successfully.",
      usableId: `form_s_corporation-${insertedId}`,
    });
  } catch (error) {
