...
const Register: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const users: User[] = JSON.parse(localStorage.getItem('tempUsers') || '[]');
  ...
  const isError = status.includes('already') || status.includes('error');
        ...
        <div className="min-h-[100dvh] flex flex-col justify-between">
          <div className="py-8 md:py-20 px-6 md:px-20 flex flex-col gap-8 items-center xl:items-start">
            ...
            <h2 className="text-black font-semibold text-3xl m-0">
              Sign Up to <span className="text-primary">VC Filling</span>
            </h2>
            ...
            <form onSubmit={handleRegister} className="w-full max-w-md space-y-4">
              ...
              {status && (
                <div className={`p-3 rounded-lg text-sm ${
                  isError ? 'bg-red-100 border border-red-200 text-red-800' : 'bg-green-100 border border-green-200 text-green-800'
                }`}>
                  {status}
                </div>
              )}
