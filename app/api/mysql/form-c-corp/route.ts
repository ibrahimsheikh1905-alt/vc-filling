  ...
  try {
    ...
    try {
      ...
      if (userResponse.user) {
        await handleUsers(userResponse.user, "form_c_corporation");
      }
    } catch (error) {
    ...
    const query = `
      INSERT INTO form_c_corporation (
        ${fields.join(", ")}
      ) VALUES (
        ${fields.map(() => "?").join(", ")}
      );
    ...
    await executeQuery(newQuery, [
      ...
      data.clientEmail ?? null,
      data.clientPhoneNumber ?? null,
      `form_c_corporation-${insertedId}`,
      getFormattedDate(),
    ]);

    return NextResponse.json({
      message: "Data inserted successfully.",
      usableId: `form_c_corporation-${insertedId}`,
    });
  } catch (error) {
