  ...
  try {
    ...
    try {
      ...
      if (userResponse.user) {
        ...
        await handleUsers(userResponse.user, "cert_good_standing");
      }
    } catch (error) {
    ...
    const query = `
      INSERT INTO cert_good_standing (
        ${fields.join(", ")}
      ) VALUES (
        ${fields.map(() => "?").join(", ")}
      );
    ...
    await executeQuery(newQuery, [
      ...
      data.email,
      data.mobilePhone,
      `cert_good_standing-${insertedId}`,
      getFormattedDate(),
    ]);

    return NextResponse.json({
      message: "Data inserted successfully.",
      usableId: `cert_good_standing-${insertedId}`
     });
  } catch (error) {
