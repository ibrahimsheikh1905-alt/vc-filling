...
export async function POST(req: NextRequest) {
  try {
    ...
    // Helpful debug logs for 500s
    console.log("/api/mysql/form-llc payload keys:", Object.keys(data || {}));
    console.log("/api/mysql/form-llc hasUserId:", !!data?.userId);
    console.log("/api/mysql/form-llc share_price/share_number:", data?.sharePrice, data?.shareNumber);
    console.log("/api/mysql/form-llc shareholder_number:", data?.shareholderNumber);

    console.log("/api/mysql/form-llc client fields:", {
...
// Define the fields we expect in the same order as the SQL query
    ...
    // When userId isn't available, we should still allow inserting a row.
    // If the DB column disallows NULL, you'll need a schema default. For now, keep it null.
    const values = [
      data.userId ?? null,
