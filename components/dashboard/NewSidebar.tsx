...
import { 
  ...
  FileText, LogOut, MessageCircle
} from "lucide-react";
import { usePathname, useRouter } from "next/navigation";
import Link from "next/link";
import { useAuth } from "@/contexts/auth";

// Same brand gradient used across the landing page
const LOGO_GRADIENT =
...
const NewSidebar: React.FC = () => {
  ...
  const getLinkStyle = (path: string): string => {
    ...
    return `flex items-center px-4 py-2.5 rounded-lg transition-all duration-300 group relative outline-none focus:outline-none focus:ring-0 ${
      isActive
